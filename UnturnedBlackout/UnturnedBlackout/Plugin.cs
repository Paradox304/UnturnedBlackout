...
public class Plugin : RocketPlugin<Config>
{
    ...
    private void OnJoining(CSteamID player, ref ESteamRejection? rejectionReason)
    {
        ...
        {
            newName = newName.Remove(Config.Base.FileData.MaxPlayerNameCharacters,
                chars - Config.Base.FileData.MaxPlayerNameCharacters);
            newName += "...";
        }
    ...
    private void OnDamageStructure(
    ...
    private void OnVoice(
        ...
        ref bool shouldAllow,
        ref bool shouldBroadcastOverRadio,
        ref PlayerVoice.RelayVoiceCullingHandler cullingHandler)
    {
    ...
    private void OnLevelLoaded(int level)
    {
        ...
        var shouldFillAfterDetach =
            typeof(ItemMagazineAsset).GetProperty("shouldFillAfterDetach", BindingFlags.Public | BindingFlags.Instance);
        var magazines = Assets.find(EAssetType.ITEM).OfType<ItemMagazineAsset>();
        foreach (var mag in magazines)
        {
            if (ignoreMags.Contains(mag.id)) continue;
        ...
        foreach (var mask in masks) isEarpiece.SetValue(mask, true);
    }
    ...
    public override TranslationList DefaultTranslations =>
        new()
        {
            ...
            {
                "Not_Enough_Currency",
                "You dont have enough {0} to make this purchase. Would you like to check out our Store?"
            },

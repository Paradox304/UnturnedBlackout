...
public class KCGame : Game
{
    ...
    public IEnumerator EndGame()
    {
        ...
        var wonTeam = BlueTeam.Score > RedTeam.Score ? BlueTeam :
            RedTeam.Score > BlueTeam.Score ? RedTeam : new(this, -1, true, 0, new());
    ...
    public IEnumerator GameEnd(KCTeam wonTeam)
    {
        ...
        foreach (var player in Players)
        {
            ...
            TaskDispatcher.QueueOnMainThread(() => Plugin.Instance.Quest.CheckQuest(player.GamePlayer,
                ...
                new()
                {
                    ...
                    { EQuestCondition.WinTags, player.KillsDenied + player.KillsConfirmed }
                }));
            ...
            Plugin.Instance.UI.SetupPreEndingUI(player.GamePlayer, EGameType.KC, player.Team.TeamID == wonTeam.TeamID,
        ...
        foreach (var player in Players) Plugin.Instance.UI.ShowKCLeaderboard(player.GamePlayer);
    ...
    public override IEnumerator AddPlayerToGame(GamePlayer player)
    {
        ...
        switch (GamePhase)
        {
            ...
            case EGamePhase.Ending:
                ...
                Plugin.Instance.UI.SetupKCLeaderboard(kPlayer, Players, Location, wonTeam, BlueTeam, RedTeam, true,
                    IsHardcore);
    ...
    public override void OnPlayerDead(Player player, CSteamID killer, ELimb limb, EDeathCause cause)
    {
        ...
        var updatedKiller = cause == EDeathCause.WATER ? vPlayer.GamePlayer.SteamID :
            cause == EDeathCause.LANDMINE || cause == EDeathCause.SHRED ? vPlayer.GamePlayer.LastDamager.Count > 0
        ...
        _ = Task.Run(async () => await Plugin.Instance.DB.IncreasePlayerDeathsAsync(vPlayer.GamePlayer.SteamID, 1));

        TaskDispatcher.QueueOnMainThread(() =>
        {
            ItemManager.dropItem(new(vPlayer.Team.DogTagID, true), vPlayer.GamePlayer.Player.Player.transform.position,
                true, true, true);
            ...
            {
                OnKill(kPlayer.GamePlayer, vPlayer.GamePlayer, cause == EDeathCause.WATER ? (ushort)0 : (ushort)1,
                    kPlayer.Team.Info.KillFeedHexCode, vPlayer.Team.Info.KillFeedHexCode);
            ...
            if (vPlayer.GamePlayer.LastDamager.Count > 0 &&
                vPlayer.GamePlayer.LastDamager.Peek() == kPlayer.GamePlayer.SteamID)
                ...
                if (assister != null && assister != kPlayer)
                {
                    ...
                    if (!assister.GamePlayer.Player.Player.life.isDead)
                        Plugin.Instance.UI.ShowXPUI(assister.GamePlayer, Config.Medals.FileData.AssistKillXP,
                            Plugin.Instance.Translate("Assist_Kill", vPlayer.GamePlayer.Player.CharacterName.ToUnrich())
                                .ToRich());
            ...
            if (!usedKillstreak && cause == EDeathCause.GUN &&
                (vPlayer.GamePlayer.Player.Position - kPlayer.GamePlayer.Player.Position).sqrMagnitude > longshotRange)
            {
            ...
            if (equipmentUsed != 0)
                OnKill(kPlayer.GamePlayer, vPlayer.GamePlayer, equipmentUsed, kPlayer.Team.Info.KillFeedHexCode,
                    vPlayer.Team.Info.KillFeedHexCode);
    ...
    public override void OnPlayerRespawn(GamePlayer player, ref Vector3 respawnPosition, ref float yaw)
    {
        ...
        if (!SpawnPoints.TryGetValue(kPlayer.Team.SpawnPoint, out var spawnPoints)) return;
        ...
        ItemJar P)
    {
        ...
        if (kPlayer.Team.DogTagID == P.item.id)
        {
            ...
            _ = Task.Run(async () =>
            {
                await Plugin.Instance.DB.IncreasePlayerXPAsync(kPlayer.GamePlayer.SteamID,
                    Config.Medals.FileData.KillDeniedXP);
        ...
        else if (P.item.id == otherTeam.DogTagID)
        {
            ...
            {
                await Plugin.Instance.DB.IncreasePlayerXPAsync(kPlayer.GamePlayer.SteamID,
                    Config.Medals.FileData.KillConfirmedXP);
                await Plugin.Instance.DB.IncreasePlayerKillsConfirmedAsync(kPlayer.GamePlayer.SteamID, 1);
            });

            TaskDispatcher.QueueOnMainThread(() =>
            {
                foreach (var ply in Players) Plugin.Instance.UI.UpdateKCScore(ply, kPlayer.Team);
        ...
        if (kPlayer.CollectorTags == Config.Medals.FileData.CollectorTags)
        {
            ...
            TaskDispatcher.QueueOnMainThread(() =>
                Plugin.Instance.Quest.CheckQuest(kPlayer.GamePlayer, EQuestType.Collector, questConditions));
        }

        Plugin.Instance.UI.ShowXPUI(kPlayer.GamePlayer, xpGained, xpText);
        TaskDispatcher.QueueOnMainThread(() =>
            Plugin.Instance.Quest.CheckQuest(kPlayer.GamePlayer, EQuestType.Dogtags, questConditions));

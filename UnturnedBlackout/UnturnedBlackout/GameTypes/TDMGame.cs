...
namespace UnturnedBlackout.GameTypes
{
    public class TDMGame : Game
    {
        ...
        public IEnumerator GameEnd(TDMTeam wonTeam)
        {
            ...
            foreach (TDMPlayer player in Players.ToList())
            {
        ...
        public override IEnumerator AddPlayerToGame(GamePlayer player)
        {
            ...
            TDMTeam team = BlueTeam.Players.Count > RedTeam.Players.Count ? RedTeam : BlueTeam;
            ...
            switch (GamePhase)
            {
                case EGamePhase.WaitingForPlayers:
                    ...
                    else
                    {
                        ...
                        foreach (TDMPlayer ply in Players)
                        {
                            if (ply == tPlayer)
                            {
                                continue;
        ...
        {
            TDMPlayer tPlayer = GetTDMPlayer(player);
            if (tPlayer == null)
            {
                return;
            }
            ...
            int victimKS = tPlayer.Killstreak;
            CSteamID updatedKiller = cause == EDeathCause.WATER ? tPlayer.GamePlayer.SteamID : (cause == EDeathCause.LANDMINE || cause == EDeathCause.SHRED ? (tPlayer.GamePlayer.LastDamager.Count > 0 ? tPlayer.GamePlayer.LastDamager.Pop() : killer) : killer);
            ...
            TaskDispatcher.QueueOnMainThread(() =>
            {
                TDMPlayer kPlayer = GetTDMPlayer(updatedKiller);
                if (kPlayer == null)
                {
                ...
                {
                    TDMPlayer assister = GetTDMPlayer(tPlayer.GamePlayer.LastDamager.Pop());
        ...
        {
            TDMPlayer player = GetTDMPlayer(parameters.player);
            if (player == null)
            {
                return;
            }
            ...
            TDMPlayer kPlayer = GetTDMPlayer(parameters.killer);
            if (kPlayer == null)
            {
                return;
            }
        ...
        public override void OnPlayerRespawn(GamePlayer player, ref Vector3 respawnPosition, ref float yaw)
        {
            ...
            if (!SpawnPoints.TryGetValue(tPlayer.Team.SpawnPoint, out List<TDMSpawnPoint> spawnPoints))
            {
                return;
            }
        ...
        public override void OnChatMessageSent(GamePlayer player, EChatMode chatMode, string text, ref bool isVisible)
        {
            ...
            TaskDispatcher.QueueOnMainThread(() =>
            {
                ...
                IEnumerable<TDMPlayer> loopPlayers = chatMode == EChatMode.GLOBAL ? Players : Players.Where(k => k.Team == tPlayer.Team);
                foreach (TDMPlayer reciever in loopPlayers)
                {

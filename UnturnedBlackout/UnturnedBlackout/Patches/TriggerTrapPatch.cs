...
public static class TriggerTrapPatch
{
    ...
    public static void Prefix(Collider other, InteractableTrap __instance)
    {
        ...
        if (!Provider.isPvP || other.transform.CompareTag("Vehicle")) return;
        ...
        if (gPlayer.SteamID.m_SteamID == data.owner ||
            (data.group != 0UL && data.group == gPlayer.Player.Player.quests.groupID.m_SteamID)) return;

    ...
    */
    ...
    public void ShowGun(LoadoutGun gun)
    {
        ...
        if ((LoadoutPage == ELoadoutPage.PRIMARY && loadout.Primary == gun) || (LoadoutPage == ELoadoutPage.SECONDARY && loadout.Secondary == gun))
            _ = Task.Run(() => ShowEquippedGunStats(loadout, gun));
        else
        {
            var currentGun = LoadoutPage == ELoadoutPage.PRIMARY ? loadout.Primary : loadout.Secondary;
            _ = Task.Run(() => ShowComparisonGunStats(currentGun, gun));
        }
    }
    ...
    public void ShowAttachment(LoadoutAttachment attachment, LoadoutGun gun)
    {
        ...
        if (attachment.IsBought && ((isAttachmentPrimary && loadout.PrimaryAttachments.ContainsValue(attachment)) || (!isAttachmentPrimary && loadout.SecondaryAttachments.ContainsValue(attachment))))
            _ = Task.Run(() => ShowEquippedAttachmentStats(loadout, gun, attachment));
        else
            _ = Task.Run(() => ShowComparisonAttachmentStats(loadout, gun, attachment));
    }
    ...
    public void ShowKnife(LoadoutKnife knife)
    {
        ...
        EffectManager.sendUIEffectText(MAIN_MENU_KEY, TransportConnection, true, "SERVER Item Owned TEXT", $"Melee owned by {knife.Knife.UnboxedAmount} players");
    ...
    public void ShowLeaderboards()
    {
        ...
        EffectManager.sendUIEffectImageURL(MAIN_MENU_KEY, TransportConnection, true, "SERVER Leaderboards Flag IMAGE 10", PlayerData.HideFlag || PlayerData.CountryCode == "NNN" ? Config.Icons.FileData.HiddenFlagIconLink : Utility.GetFlag(PlayerData.CountryCode));
    ...
    public void ShowLeaderboardPage(int pageNum)
    {
        ...
        for (var i = lowerIndex; i <= upperIndex; i++)
        {
            ...
            EffectManager.sendUIEffectImageURL(MAIN_MENU_KEY, TransportConnection, true, $"SERVER Leaderboards Flag IMAGE {index}", playerData.HideFlag || playerData.CountryCode == "NNN" ? Config.Icons.FileData.HiddenFlagIconLink : Utility.GetFlag(playerData.CountryCode));
    ...
    public void SearchLeaderboardPlayer(string input)
    {
        ...
        for (var i = 0; i < maxCount; i++)
        {
            ...
            EffectManager.sendUIEffectImageURL(MAIN_MENU_KEY, TransportConnection, true, $"SERVER Leaderboards Flag IMAGE {i}", playerData.HideFlag || playerData.CountryCode == "NNN" ? Config.Icons.FileData.HiddenFlagIconLink : Utility.GetFlag(playerData.CountryCode));
    ...
    public void ShowAchievements()
    {
        ...
        EffectManager.sendUIEffectText(MAIN_MENU_KEY, TransportConnection, true, "SERVER Achievements Page 2 TEXT", "General");
        EffectManager.sendUIEffectVisibility(MAIN_MENU_KEY, TransportConnection, true, "SERVER Achievements Page 3 BUTTON", true);
        EffectManager.sendUIEffectText(MAIN_MENU_KEY, TransportConnection, true, "SERVER Achievements Page 3 TEXT", "Event");
